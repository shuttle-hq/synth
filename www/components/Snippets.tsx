...
import { pgGenerate } from "../lib/playground"
  ...
  return (
    <div className="flex lg:flex-row flex-col">
      <div className="flex-grow rounded bg-gray-600 lg:mr-16">
        <div className="font-semibold bg-gray-600 rounded-lg rounded-br-none rounded-bl-none py-2 pl-4 pr-2 border-2 border-t-0 border-r-0 border-l-0 border-gray-500 text-gray-300">
          <div className="flex flex-row">
            <div className="flex-grow my-auto">
              <FontAwesomeIcon
                className="inline-block h-3.5 pb-1 pr-1"
                style={{ color: "#6422f1" }}
                icon={faCircle}
              />
              <FontAwesomeIcon
              ...
              />
              <FontAwesomeIcon
                className="inline-block h-3.5 pb-1 pr-4"
                style={{ color: "#2bd38d" }}
                icon={faCircle}
              />
              {title}
            ...
            <div
              ...
              onClick={() => {
                pgGenerate(codeString, 4)
                  ...
                  .catch((err) =>
                    console.log("failed to run playground query: " + err)
                  )
              }}
            >
              <AccentButton compact>
                <FontAwesomeIcon
                  icon={output ? faRedo : faPlayCircle}
                  className="h-4 pr-2 pt-0.5"
                />
                {output ? "Re-run" : "Run"}
              </AccentButton>
            </div>
        ...
        {!output && (
          <PrettyCode
            code={codeString}
            className={output && "border-b-2 border-gray-500"}
            lineNumbers
          />
        )}
      ...
      <div className="lg:pt-0 pt-16 text-gray-300 lg:w-80">
        <div className={`grid grid-rows-${length} gap-2`}>{tabs}</div>
      </div>
    </div>
  )
}
