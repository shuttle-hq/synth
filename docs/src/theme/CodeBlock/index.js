...
export default (({
    ...
    const defaultPrismTheme = usePrismTheme(); // In case interleaved Markdown (e.g. when using CodeBlock as standalone component).
    const highlightPrismTheme = {
        ...defaultPrismTheme, ...{
            "plain": {
                ...defaultPrismTheme["plain"],
                ...{
    ...
    const prismTheme = !isResult ? defaultPrismTheme : highlightPrismTheme;
    ...
    let codeWithoutComments = code.replace(/\/\/.*/g, '');
    ...
    return <>
        <Highlight {...defaultProps} key={String(mounted)} theme={prismTheme} code={code} language={language}>
            {({
                ...
                <div className={`${styles.codeBlockContent} ${isResult ? styles.resultBlockContent : ""}`}>
                    <div
                    ...
                    <div className={clsx(styles.buttonsBlock)}>
                        {
                            !isResult &&
                            <button ref={copyButton} type="button" aria-label="Copy code to clipboard"
                                    className={`${clsx(styles.codeButton)} ${clsx(styles.copyButton)}`}
                        ...
                        }
                        {
                            ...
                            <button ref={runButton} type="button" aria-label="Run code with Synth"
                        ...
                        {
                            isResult &&
                            <span className={clsx(styles.codeButton)}>
                                Output
                            </span>
                        }
                    </div>
