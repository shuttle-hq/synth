...
image: https://commons.wikimedia.org/wiki/File:Dialog-error-round.svg#/media/File:Dialog-error-round.svg
...
\#3 It's not that long that we can use literal numbers for number constants here (for example given the `length`). The old way would use a [`number`](https://www.getsynth.com/docs/content/number) generator. A recent improvement let us generate arbitrary numbers, however this is likely not a good idea for a `length` field:
...
\#5 Ok, let's make that a [`format`](https://www.getsynth.com/docs/content/string#format) then. However, I forgot that the `format`must contains a map with the keys "format" and "arguments", putting them into the `s` map directly:
...
\#9 Now we let `date_time` have some rest and go on to [`categorical`](https://www.getsynth.com/docs/content/string#categorical). Having just one variant with a weight of `0` will actually trigger an `unreachable` error:
